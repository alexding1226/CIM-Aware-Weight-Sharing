...
class BlockDS(nn.Module):
    ...
    def compute_dist_qkv(self,qkv_mask):
        ...
        for head_idx in range(num_heads-1):
            ...
            # q_dist = torch.sum(compute_column_distances(q_weight[head_idx*head_dim:(head_idx+1)*head_dim,:],q_weight[(head_idx+1)*head_dim:(head_idx+2)*head_dim,:])* qkv_mask[0][head_idx])
            # k_dist = torch.sum(compute_column_distances(k_weight[head_idx*head_dim:(head_idx+1)*head_dim,:],k_weight[(head_idx+1)*head_dim:(head_idx+2)*head_dim,:])* qkv_mask[1][head_idx])
            # v_dist = torch.sum(compute_column_distances(v_weight[head_idx*head_dim:(head_idx+1)*head_dim,:],v_weight[(head_idx+1)*head_dim:(head_idx+2)*head_dim,:])* qkv_mask[2][head_idx])
...
class RSVisionTransformer(nn.Module):
    ...
    def set_mask(self,qkv_mask,fc1_mask=None,fc2_mask=None):
    ...
    def compute_dist_qkv(self,qkv_mask):
        ...
        #     block_dist = block.compute_dist_qkv(qkv_mask[idx])
