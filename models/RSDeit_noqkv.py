...
class RSVisionTransformer(nn.Module):
    ...
    def forward(self, x):
        x, qkv_list = self.forward_features(x)
    ...
    @torch.no_grad()
    def grad_share_qkv(self,idx_mapping_list):
        head_num = 12
        head_dim = self.embed_dim // head_num
        for idx, block in enumerate(self.blocks):
            q_grads = block.attn.qkv.weight.grad[:self.embed_dim,:]
            k_grads = block.attn.qkv.weight.grad[self.embed_dim:2*self.embed_dim,:]
            v_grads = block.attn.qkv.weight.grad[2*self.embed_dim:,:]
            ...
            for share_idx in q_share_idx:
                ...
                for id in share_idx[2:]:
                    grad += q_grads[nxt_head*head_dim :(nxt_head+1)*head_dim, id]
                    nxt_head = nxt_head+1
                
                for i in range(len(share_idx)-1):
                    head_id = start_head + i
                    q_grads[head_id*head_dim : (head_id+1)*head_dim, share_idx[i+1]] = grad
            ...
            for share_idx in k_share_idx:
                ...
                nxt_head = start_head+1
                for id in share_idx[2:]:
                    grad += k_grads[nxt_head*head_dim :(nxt_head+1)*head_dim, id]
                    nxt_head = nxt_head+1
                
                for i in range(len(share_idx)-1):
                    head_id = start_head + i
                    k_grads[head_id*head_dim : (head_id+1)*head_dim, share_idx[i+1]] = grad
            ...
            for share_idx in v_share_idx:
                ...
                nxt_head = start_head+1
                for id in share_idx[2:]:
                    grad += v_grads[nxt_head*head_dim :(nxt_head+1)*head_dim, id]
                    nxt_head = nxt_head+1
                
                for i in range(len(share_idx)-1):
                    head_id = start_head + i
                    v_grads[head_id*head_dim : (head_id+1)*head_dim, share_idx[i+1]] = grad

            block.attn.qkv.weight.grad[:self.embed_dim,:] = q_grads
            block.attn.qkv.weight.grad[self.embed_dim:2*self.embed_dim,:] = k_grads
            block.attn.qkv.weight.grad[2*self.embed_dim:,:] = v_grads
